        ...
        {events.map(e=>{
          ...
          return(
            <div key={e.id} className="flex items-center justify-between p-3 rounded-xl border" style={{background:isAssigned?"#f0fdf4":"#f8fafc",borderColor:isAssigned?"#86efac":"#e5e7eb"}}>
              ...
              <button onClick={async()=>{setSaving(e.id);try{if(isAssigned){await coordEventsDB.unassign(coord.id,e.id);onShow("Unassigned");} else {await coordEventsDB.assign(coord.id,e.id);const commission=await coordCommissionsDB.getOrCreate(coord.id,coord.name,e.id,e.name,coord.commissionRate);try{await coordCommissionsDB.updateRevenue(e.id);}catch(err){}onShow("Assigned!");}reload();}catch(err){onShow("Error: "+err.message);}setSaving(null);}} disabled={saving===e.id} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${isAssigned?"bg-red-50 text-red-600 hover:bg-red-100":"text-white"}`} style={!isAssigned?{background:NAVY}:{}}>{saving===e.id?<Spinner size="sm"/>:isAssigned?"Unassign":"Assign"}</button>
            </div>
          );
        })}
...
function RepLogSale({repInfo,onSuccess}){
  ...
  const submit=async e=>{
    ...
    try{
      ...
      if(!pricePerTix||pricePerTix<=0){setError("This allocation has no ticket price set. Ask an admin/coordinator to re-allocate it with a ticket type selected.");setLoading(false);return;}
